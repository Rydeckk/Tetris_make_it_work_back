...
export class SkillsService {
    ...
    async affectSkillUser(skillData: CreateUserSkill) {
        return this.prisma.usersSkills.create({
            data: {
    ...
    async affectSkillTask(skillData: CreateSkillTask) {
        // Check if the task already has 5 skills
        const existingSkillsTasks = await this.prisma.skillsTasks.findMany({
            where: { taskId: skillData.taskId },
        });

        if (existingSkillsTasks.length >= 5) {
            throw new BadRequestException('A task cannot have more than 5 skills.');
        }

        return this.prisma.skillsTasks.create({
            data: {
                skillId: skillData.skillId,
    ...
    async changeSkillUser({ skillId, user, weight }: CreateUserSkill) {
        return this.prisma.usersSkills.update({
            where: { userId_skillId: { userId: user, skillId } },
